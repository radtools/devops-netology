...
```
 ...
 ``cost`` -затратность операции. первое значение (0.00) -затраты на получение первой строки, второе (12.40) - затраты на получение всех строк. ``rows`` - примерное кол-во возвращаемых строк при ``Seq Scan``, ``width`` — средний размер одной строки в байтах.
